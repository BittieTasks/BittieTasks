...
async function sendEmailViaSendGrid(params: {
  ...
  try {
    ...
    if (error.code === 403) {
      return { success: false, error: 'SendGrid sender verification required. Please verify your sender email in SendGrid dashboard.' }
    }
...
export class EmailVerificationServiceFixed {
  ...
  private async ensureTable(): Promise<boolean> {
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        console.log('email_verification_tokens table not found - using fallback approach')
      }
...
export const emailVerificationFixed = new EmailVerificationServiceFixed()