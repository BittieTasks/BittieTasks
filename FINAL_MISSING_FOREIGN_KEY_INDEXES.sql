-- Final Missing Foreign Key Indexes for BittieTasks
-- These are the CRITICAL indexes that will eliminate unindexed foreign key warnings
-- These are different from unused indexes - these MUST be added for optimal performance

-- User achievements and challenges (missing foreign key indexes)
CREATE INDEX IF NOT EXISTS idx_user_achievements_user_id ON public.user_achievements(user_id);
CREATE INDEX IF NOT EXISTS idx_user_challenges_user_id ON public.user_challenges(user_id);

-- Task participants (if still missing)
...
-- They eliminate "unindexed foreign keys" warnings and improve join performance