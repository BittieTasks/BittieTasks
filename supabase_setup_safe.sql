-- Safe BittieTasks Database Setup for Supabase
-- This script handles existing tables and types gracefully

-- Drop existing types if they exist (safe approach)
DROP TYPE IF EXISTS "task_status" CASCADE;
DROP TYPE IF EXISTS "task_type" CASCADE;
DROP TYPE IF EXISTS "approval_status" CASCADE;
DROP TYPE IF EXISTS "review_tier" CASCADE;
DROP TYPE IF EXISTS "transaction_type" CASCADE;
DROP TYPE IF EXISTS "transaction_status" CASCADE;
DROP TYPE IF EXISTS "verification_method" CASCADE;
DROP TYPE IF EXISTS "verification_status" CASCADE;
DROP TYPE IF EXISTS "revenue_stream" CASCADE;
...
DROP TABLE IF EXISTS "task_completion_submissions" CASCADE;
DROP TABLE IF EXISTS "task_verification_requirements" CASCADE;
...
-- Everyone can read categories and tasks
...
CREATE POLICY "Users can create tasks" ON "tasks" FOR INSERT TO authenticated WITH CHECK (auth.uid()::text = created_by);
CREATE POLICY "Users can update own tasks" ON "tasks" FOR UPDATE TO authenticated USING (auth.uid()::text = created_by);

-- Task participants policies
CREATE POLICY "Anyone can view task participants" ON "task_participants" FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can apply to tasks" ON "task_participants" FOR INSERT TO authenticated WITH CHECK (auth.uid()::text = user_id);
CREATE POLICY "Users can update own participation" ON "task_participants" FOR UPDATE TO authenticated USING (auth.uid()::text = user_id);

-- Phone verification policies
CREATE POLICY "Users can view own verification codes" ON "phone_verification_codes" FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can create verification codes" ON "phone_verification_codes" FOR INSERT TO authenticated WITH CHECK (true);

-- Database setup complete!
SELECT 'BittieTasks database setup completed successfully!' as status;